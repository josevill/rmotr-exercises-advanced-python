from libs import exceptions_tictactoe
...
def move(game, player, position):
    ...
    if game['winner'] or _board_is_full(board):
        raise  exceptions_tictactoe.InvalidMovement('Game is over.')

    if player != get_next_turn(game):
        raise  exceptions_tictactoe.InvalidMovement('"{}" moves next.'.format(game['next_turn']))

    if not _position_is_valid(position):
        raise  exceptions_tictactoe.InvalidMovement('Position out of range.')

    if not _position_is_empty_in_board(position, board):
        raise  exceptions_tictactoe.InvalidMovement('Position already taken.')
    ...
    if winner:    #record results section
        game['winner'] = winner
        game['next_turn'] = None
        raise exceptions_tictactoe.GameOver('"{}" wins!'.format(winner))
    elif _board_is_full(board):    # board is full AND no winner...
        game['next_turn'] = None
        raise exceptions_tictactoe.GameOver('Game is tied!')
...
def _check_winning_combinations(board, player):
    ...
    for combination in combinations:
        # Use winning combination helper to check if there's
        # any combination in the board and return the player as a winner
